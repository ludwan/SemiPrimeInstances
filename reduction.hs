...
mainAlg tal ad mult= 
 do args <- getArgs
    ...
    putStr ("c Problem generated from " ++ mult ++ "["
            ++ ad ++ "] multiplication circuit\n")
    ...
    print (reverse bs)
    appendFile "UnsafeSameSize35reduktion.txt" ("c " ++ show tal ++ "\n")
    appendFile "UnsafeSameSize35reduktion.txt" "p cnf "
    appendFile "UnsafeSameSize35reduktion.txt" (show last ++ " " ++ show (length (filter cnf_pred cnf)) ++ "\n")
    appendFile "UnsafeSameSize35reduktion.txt" (concat (intersperse "\n" (map show (filter cnf_pred cnf))))
    appendFile "UnsafeSameSize35reduktion.txt" "\n"
    appendFile "UnsafeSameSize35reduktion.txt" "-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/\n"
...
main :: IO ()
main = do 
       inh <- openFile "UnsafeSameSize35" ReadMode
    ...
    do ineof <- hIsEOF inh
       if ineof
           then return ()
                   ...
                   mainAlg (read inpStr :: Integer) "n-bit" "carry-save"
                   mainloop inh outh
